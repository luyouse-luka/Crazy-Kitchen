...
## 📍 当前断点（2026-09-18 第十一轮 · 每轮收工更新这一段）

**⏸ 第十一轮：真机第一次出画面。黑屏、点不动、镜头太远，三件都修完了 ——
M2 从「拖上去跑一次」变成「真机上能玩」。**

全程通过 MCP 直接改你本机的 Cocos 工程（反向 SSH 隧道），服务器这份只跑判据。
**你本地有一批未提交的改动，push 之前服务器拿不到。**

### ✅ 这一轮落地的

| | 症状 | 真因 | 判据 |
|---|---|---|---|
| 黑屏 | 真机 / 模拟器全黑 | 构建**只打包了 `test.scene`**（一个空场景），起始场景也指着它 | `src/settings.<hash>.json` 的 `launchScene` = `db://assets/main.scene` |
| Error 3817 | `Main Camera, 1` | `test.scene` 的相机上挂着一个 `cc.MissingScript` | 移除后控制台无新错 |
| 面板点不动 / 摇杆飘偏 | 点冰箱格子无反应，白块不在手指下 | **捕获区算在设计单位里，手指活在物理像素里**，实测差 1.978 倍 | `tests/input.test.ts` 新增 2532×1170 一组 + 一条 `designH` 漏传的反例 |
| 镜头太远 | 「两端全是黑色，人物显得小」 | 正交半高 5 → 画面 21.6m 宽，场景横向投影才 11.5m，**只填满 53%** | `pnpm cam`（新） |
| `UI_HUD` 层 | — | layer 是 `DEFAULT`，Canvas 和所有兄弟都是 `UI_2D` | `pnpm scene` |

### 镜头那一条：数值是算出来的，不是调出来的

`pnpm cam` 扫 3264 个玩家可达位置，把三件互相拉扯的事同时摆出来：

```
半高  横向填充  人物占屏  玩家余量  四工位全见
2.6   100%      34%       0.04m ←贴边   8%
3.0    89%      29%       0.74m        34%   ← 取这档
3.4    78%      26%       0.81m        90%
5.0    53%      18%       1.37m       100%   ← 改之前
```

取 3.0：黑边从 47% 压到 11%，人物放大 1.67 倍，玩家还留 0.74m 不贴边。
**跟随只在画面垂直方向真的动** —— 水平方向场景本来就刚好装满，动一下只会露背景色，
所以 `focusBounds()` 把水平那维算成一个点。这不是偷懒，是 `CONTENT_SPAN` 推出来的。

钳制放在**相机视图坐标** `(u, v)` 里而不是世界 `(x, z)` 里：相机 yaw −45，
画面的两条轴在世界里是斜的，世界 AABB 表达不了「这维装得下别动、那维装不下跟住」。

⚠ **4:3 平板是另一档**：画面比场景还窄，跟随会把烤炉甩出画面（实测 41% 的位置看不见它）。
`effectiveOrthoHeight()` 在那种比例下把半高抬到装得下整个场景宽，退回全景、水平不动。

### ⚠ 两个「git 里看不见」的配置，是黑屏的真正温床

1. **构建配置在 `game/profiles/v2/packages/builder.json`，而 `game/profiles/` 是 gitignored 的。**
   起始场景、要打包哪些场景，全在这里。新克隆一份、或换台机器，这些值回到默认 ——
   **黑屏会原样复现，而 `git diff` 什么都不显示**。改它也不能直接写文件：
   Build 面板在内存里另存一份，构建时反向盖回磁盘，得走 `query-task` → `update-task` → `save-task`。
2. **设计分辨率原本也只活在编辑器内存里**（`project.json` 磁盘上只有一行版本号）。
   已显式落盘成 `1280×720 / fitWidth:true`，进了仓库。这条不落盘的话，
   换台机器打开会退回引擎默认值，UI 与捕获区整片偏移。

### 「场景为什么是个菱形」—— 不是必须的，但也不是相机的错

这一问值得单独记，因为**我的第一直觉是错的，数据把它推翻了**。

直觉：菱形是相机转了 45° 造成的，转回正就好。实测（`pnpm cam` 的「视角对比」一节，
逐像素投射视线、算画面被场景盖住的比例）：
...
**转正反而更差。** 45° 斜视把场景在画面上的跨度从 8m 拉到 11.5m，正好更贴宽屏。
而且无论怎么转，覆盖率天花板只有 69% —— 说明空的那 30% 根本不是角度问题：
**画面在地面上铺开 136㎡，场景本体只有 64㎡，装不满。**

解法是给场景加地，不是转相机。正交投影没有地平线，视线全都以同一个俯角射向地面，
所以地够大，每条视线都打在地上：加一块 24×24 的 `Floor_Outer`（纯装饰，
不参与碰撞也不进相机跟随边界），**覆盖率 63% → 100%**。

顺带证否了两条：补一面西墙只值 1pt；改俯角最多值 6pt。都不值得为它们改场景。

### 适配策略：定 Fit Height，但不是这一轮

`docs/m2-scene-guide.md` 当初写的是 Fit Height，工程实跑的一直是 Fit Width
（从没配过，吃的是引擎默认值）。**结论：Fit Height 对，横屏游戏该锁高度让宽度延展**，
宽屏能看到更多而不是被裁掉更多，摇杆和动作键也能拉得更开。

**但不在这一轮改。** 它要连着 UI 一起动（`UI_HUD` 现在是写死的 1280×720，
Fit Height 下宽屏两侧会各露出 139px，得改成 Widget 四边贴 0），而且要重验一轮真机触控 ——
而触控刚在 Fit Width 下验通过。这一轮已经动了相机和场景，再叠一个变量，出问题就分不清是哪一个。
排进下一轮，单独验。

### 顺带修的（你授权的「顺带发现也一同修」）

- `game/assets/logic/__tests__/` 下 12 个孤儿 `.meta` —— 测试早搬去 `tests/` 了（commit `4ea1068`），
  meta 没跟着删，编辑器每次启动都为它们找不到源文件
- `logic/input.ts` 那句 `// Fit Height: scale comes from height` 是错的，**正是它把上一轮带沟里的**
- `tests/input.test.ts` 那组「真机分辨率」用例用的是 Fit Height 的数字（2400×1080, k=1.5），
  且不传 `designH`，所以**恰好测不出**这一轮修的那个 bug。已换成 2532×1170 并补反例

### M2 闭环合上了：顾客 → 订单 → 上菜 → 打烊 → 结算

这一段是「30 秒玩完一局，有始有终有结算」那条验收判据的正身。局长定 **60 秒**
（30 秒只来得及 2 个顾客，体感不出「忙」；正式局长 210s 留给 M4）。

**`customer.ts`（新）—— 顾客流从 `sim.ts` 里抽出来，两边共用。**
这一步不是为了好看：各写一份的话，M1 标定出来的难度曲线对真游戏就不成立，
而这种偏差要真机玩几十局才看得出来。抽完跑 `pnpm layout`，
**完成率 / 后期 / idle / 偏差四个数与重构前逐位相同** —— RNG 序列没被动过，这才算抽对了。

⚠ 抽的过程中翻出一个真 bug：`rollOrder` 原地洗牌池，而 reset 从来没还原过它，
所以**重开一局同一个 seed 出的是另一批单**。`sim.ts` 的 `runDay` 每次新建状态，
所以这个坑从没暴露过；真人局按「再来一局」就会踩到。

**`shift.ts`（新）—— 一局**：计时、上菜记账、结算。上菜那一下由组件把两边接起来：
`matchCustomer()` 挑人 → `interact(serve, {spec})` 判定 → `settleServe()` 记账。
规则：先找吃得下这一盘的，找不到**砸在最急的那位头上** —— 没有匹配也一定要有人接，
否则做错了没有代价，`banned` 那一维白设计；上错菜顾客照样走，
不然玩家会拿出餐口当试错工具，一单一单试到对为止。

**UI**（场景里新建 25 个节点）：顶部 6 张订单卡（食材 + 火候 + 忌口 + 耐心条）、
倒计时、完成数；打烊弹结算面板（星级 / 来客 / 完成 / 上错 / 跑单 / 完成率 / 再来一局）。

⚠ **建 UI 时踩到三个只有判据抓得住的坑**，三条都是「画面看着没毛病」那一类：

1. **`Sprite` 赋 `spriteFrame` 会按图片原始尺寸把 `contentSize` 顶回去**（40×36），
   `sizeMode` 必须**在赋图之前**切 CUSTOM。15 个节点全中。画面上只是「背景块小了点」，
   但 `Btn_Again` 的**捕获区按 40×36 算 —— 真机上点不中**。
   抓到它是因为把那三个尺寸写进了 `UI_SPEC`。
2. **节点撞名**：结算面板里那个 Label 我起名 `Body`，而角色身体也叫 `Body`。
   `pnpm scene` 按名字找节点，抓错了一个，报「Body Position (0,8,0) ≠ 定稿 (0,0.6,0)」。
   已改名 `Stats`。⚠ 场景里起名前先 grep 一遍 `scene-spec.ts`。
3. **判据自己会失守**：`router.tick()` 那条顺序判据取的是 `indexOf` 的**第一处**。
   我在结算分支里单独调了一次 tick，它就报「排在读脉冲之前」——
   代码其实两个分支都对，但判据表达不了。已把 tick 合并成一处，
   并给判据加了一条「有多处就先红」，免得下次它静默读绿。

### 冰箱可以换食材了

点错一样食材不该逼玩家先跑一趟垃圾桶 —— 那趟路在一局里是实打实的惩罚，而错因只是眼花。
`takeFromFridge` 现在手上拿着生料直接换掉。**盘子是唯一例外**：那是组装好的汉堡，
换食材等于整个扔掉，代价和「拿错一片生菜」不是一回事，要丢得自己按 discard。

### ⏳ 下一步

1. **你 commit + push** —— 服务器这份没有 `Floor_Outer`、那 25 个 UI 节点和
   `M_FloorOuter.mtl`，`pnpm scene` 会一直报缺到同步为止
2. 换 Fit Height + `UI_HUD` 改全屏 Widget，单独验一轮真机
3. 顾客还只是订单卡，场上没有人。M3 做顾客系统时把人摆进顾客区 —— 顺带填掉画面右侧那块空地

### 第十一轮新增/改动（`project/kitchen-chaos/` 下）

| 文件 | 改动 |
|---|---|
| `game/assets/logic/camera.ts` | **新** 相机跟随，零引擎依赖。视图坐标钳制 + 窄屏兜底 |
| `game/assets/logic/customer.ts` | **新** 顾客流。从 sim.ts 抽出，两边共用；修了洗牌池不复位的 bug |
| `game/assets/logic/shift.ts` | **新** 一局：计时 / 上菜记账 / 结算 |
| `game/assets/logic/sim.ts` | 顾客流改用 customer.ts；难度曲线逐位不变 |
| `game/assets/logic/kitchen.ts` | 冰箱允许换食材（盘子除外）· 新增 `resetKitchen` |
| `game/assets/logic/difficulty.ts` | `starsFor` 加局长参数，短局按比例缩星级线 |
| `game/assets/logic/types.ts` | 食材 / 火候的中文显示名 |
| `game/assets/logic/API.md` | v0.7：补 camera / customer / shift 三节 |
| `game/assets/logic/input.ts` | 改错注释（Fit Height → Fit Width），换算本身没动 |
| `game/assets/scripts/StationView.ts` | `syncScreen()` 改读物理像素 + `designH`；接上相机跟随与运行时 `orthoHeight` |
| `game/assets/main.scene` | 相机 `_orthoHeight` 5 → 3；`UI_HUD` layer → `UI_2D`；新增 `Kitchen/Floor_Outer` |
| `game/assets/materials/M_FloorOuter.mtl` | **新** 外围地面材质，(85,92,105) 冷灰，比店内暗两档 |
| `game/assets/test.scene` | 移除相机上的 `cc.MissingScript`（Error 3817 的源头） |
| `game/settings/v2/packages/project.json` | **新增内容** 设计分辨率显式落盘 |
| `game/assets/logic/__tests__/` | **删** 12 个孤儿 meta |
| `tools/camcheck.ts` | **新** `pnpm cam` 相机几何判据 + 视角对比（逐像素投射视线算画面覆盖率） |
| `tools/scenedump.ts` | tick 顺序判据加「多处调用先红」，防它只认第一处而静默失守 |
| `tools/scene-spec.ts` | 加 `Floor_Outer` 与它的材质 |
| `tests/camera.test.ts` | **新** 8 条，守正逆变换互反、水平不跟随、窄屏兜底 |
| `tests/customer.test.ts` | **新** 13 条，守到达节奏、耐心、同 seed 可复现、matchCustomer 三种情形 |
| `tests/shift.test.ts` | **新** 11 条，守打烊硬边界、上菜记账、重开一局、星级缩放 |
| `tests/kitchen.test.ts` | 冰箱换食材的三条（含盘子仍被挡） |
| `tests/input.test.ts` | 真机那组换成 Fit Width 数字，补 `designH` 漏传反例 |
| `package.json` | 加 `cam` / `precam` |
| `docs/m2-scene-guide.md` | 适配策略三处改到与现实一致，加「待拍板」一节 |

`pnpm check` 全绿：铁律① 0 命中 · typecheck 无错 · **292 测试 / 16 文件**。
`pnpm scene` 全绿 · `pnpm cam` 全绿（四种屏幕比例下玩家与烤炉都完整可见）。
真机构建：`launchScene = db://assets/main.scene`，19.6s。

---

## 第十轮存档（2026-09-15 · 组件层接线）
...
| | 定了什么 | 落在哪 |
|---|---|---|
| 冰箱选 8 种食材 | 走到冰箱 → 点动作键 → **弹面板选** | `UI_FridgePanel` + 8 个 `Slot_*` |
| 丢弃触发 | 手里有东西时右下角**多一个丢弃键**，**长按 0.3s** | `UI_DiscardButton` |
| 游戏名 | **暂定「疯狂大厨」**（⚠ 不在原候选表里，且你说的是「暂定」） | 见下 |

**面板打开时世界继续跑** —— 烤炉继续烤、顾客耐心继续掉。这条定死了两件事：
面板**不能全屏遮挡**（玩家要看得见正在糊的那块肉），以及它不能被拿来当暂停键用。

### 这一轮挖出来的坑：两条需求都撞上 `TouchRouter` 的核心假设

`input.ts` 的分路是**按左右半屏**的，不是按钮矩形命中 —— 右半屏**任何一点**按下都算动作键。
所以照原样做的话：

- 丢弃键在右下角 → 按它的同时**必然又触发一次动作键**（在冰箱前就是又取一次料）
- 面板 8 格横跨屏幕中线 → 左两列推摇杆、右两列按动作键

解法是给 `TouchRouter` 加**捕获区**：登记的矩形在 `onDown` 时优先认领手指，认领后不进左右分路。
一个机制同时喂饱丢弃键和 8 个格子 —— 这也是丢弃键最终没做成「空地长按」的原因：
捕获区因面板已成必需，丢弃键只是多加一个 zone，边际成本近零，
而「空地长按」省不掉那笔改动，却要付「隐藏手势没人发现」的代价。

⚠ **这反转了 `input.ts` 写死的一条既往决策**（「两个键会挤占右下角……一个键够用」）。
反转理由：丢弃键**只在 `carry.kind !== 'none'` 时显示**，不是常驻，「挤占」的前提不成立。
可回退方案仍留着 —— 真机上若右下角确实挤，改回「不在任何工位范围内时长按动作键」，
零新按钮、零 `TouchRouter` 改动。

### ⚠ 差点写进去的一个真机 bug：捕获区的坐标系

`docs/m2-scene-guide.md` 里那条「`splitX` 别写死 640」救了一次 ——
**同一条坑捕获区也踩，而且更隐蔽**：

- 原点差半屏：UI 节点 position 是 Canvas 中心系（x∈[-640,640]），触摸是左下原点
- 单位差一个缩放：Fit Height 下 2400×1080 的手机 scale = 1.5，**按 1280 硬算全部错位**

要命的是**编辑器 1280×720 预览里完全正常，只有真机会错**，且错的样子像「按钮没反应」
...
3. **建 `UI_DiscardButton`** —— Sprite 120×120，`Widget` 锚右下 `right=140` `bottom=300`，`active` 关掉
4. **建 `UI_FridgePanel` + 8 个 `Slot_*`** —— 面板 560×288，8 个格子各 120×120
   （Sprite 的 `SizeMode` 必须切 **CUSTOM**），建完跑 `pnpm scene:apply --write` 自动写位置，
   **别手摆坐标**。⚠ `Slot_0..7` 的顺序就是 `INGREDIENTS` 的顺序，摆反了会「点生菜给培根」
   且编辑器里看不出来

做法与判据全在 `docs/m2-scene-guide.md` §2.1 / §2.3 的 `Canvas` 底下那张表。

### ⏳ 待你拍板：影子的 Alpha（第七轮原样挂着）

`pnpm scene` 现在报材质是 **128**、定稿是 **76**。128 是在 alpha test 还开着时选出来的，
**没有参考价值** —— 那时它只是「刚好过 0.501 阈值」，不是「看着刚好」。
现在换成真正的连续透明度，**76 第一次会被真的画出来**。重新看一眼再定，
定完告诉我，我改 `tools/scene-spec.ts` 与 `docs/m2-scene-guide.md` 三处。

### ⏰ 游戏名：暂定「疯狂大厨」，还没定死

⚠ 三件事得说清楚：

1. **它不在原候选表里**（原表是「疯狂**后厨**」/ AI后厨 / 神经病餐厅 / 厨神营业中）。
   ROADMAP 全文的标题与候选表暂时不改 —— 定死那天一次性改，免得半途留下两套名字
2. **你说的是「暂定」**，而软著要的是定死。⏰ 时钟没变：**9 月底到 10 月初**
3. ⚠ **提软著前先查一次重** —— 「疯狂大厨」这类名字同名手游不少，
   软著证书上的名称必须与上架名完全一致，撞车了要重新申请 = 推迟两三个月

### ⏳ 还没做的：组件层接线（等线 A 摆完场景）

逻辑层与判据都就位了，剩下的是 `StationView` 那一层：
...
### 第八轮新增/改动（`project/kitchen-chaos/` 下）

```
game/assets/logic/input.ts   ← 改 · 加 CaptureZone / setCaptureZones / zone()；
                                    onDown/onMove/onUp/tick/cancelAll 都走捕获区；
                                    tick 抽出 advance()，动作键与各区共用同一套点按长按语义
tests/input.test.ts          ← 改 · 23 → 52 测试（捕获区优先 / 滑出取消 / 记账边界 /
...
`pnpm check` 全绿：铁律① 0 命中 · typecheck 无错 · **233 测试 / 13 文件**（原 208）。

**证伪过的**（不是「写完就绿」）：摘掉捕获区优先 → 10 条转红 · 摘掉滑出取消 → 3 条 ·
边界改成闭区间 → 1 条 · `PANEL.gap` 改负数（格子重叠）→ 2 条 ·
面板挪出中线 → 2 条 · 格子放大到越界 → 1 条。

---

## 第七轮存档（2026-09-07 · M2 角色控制器）
...
```
docs/m2-scene-guide.md       ← 新增 · M2 场景搭建指南（编辑器侧的操作面，ly 对着做）
game/assets/logic/input.ts   ← 新增 · M2 输入层：触摸路由 + 浮动摇杆 + 点按/长按 + 相机相对映射
tests/input.test.ts          ← 新增 · 23 个单测，锁住归属锁定/死区/饱和/tap-hold/旋转不变长
game/assets/logic/API.md     ← 改 · 加 input.ts 一节（含三条真机必验）；夹具路径 __tests__ → tests
pipeline/prompt.ts           ← 改 · 三条铁律 → 四条，新增④「同执念多种机制落法」
ROADMAP.md                   ← 改 · 本段 + M2 清单勾掉两条 + 修正 A2/#0 两处过期状态
```
...
| 7 | 发布路径改到 `..\..\wechatgame` | ⬜ 只影响我能不能自动量包体，不影响构建本身 |

### A8 · 测试机（2026-09-02 盘点完毕）

| 机器 | 当什么基准 | 要提前知道的 |
|---|---|---|
| **iPhone 6s** | ⭐ **最有价值的一台**。2GB 内存 → 高性能模式下内存上限只有 **1GB**，是 iOS 闪退风险的最坏情况 | 最高只能到 iOS 15，而 §2.7 记着 **iOS 15.0–15.3 上 WebGL2 不可靠** —— 在它上面测出渲染问题，先确认系统版本再当 bug 查 |
| **iPhone 16 Pro** | 高端上限。确认画质能开到什么程度 | — |
...
> 📄 **操作指南：[`docs/m2-scene-guide.md`](docs/m2-scene-guide.md)** —— 层级树 / 相机参数 /
> 材质 / 尺寸 / 编辑器技巧 / 常见坑 / 验收判据，对着做即可。下面四条是它的目录。

- [ ] 斜 45° 相机装配：正交投影起步，角度与高度做成 Inspector 可调，真机上调到「一眼看全厨房」（指南 §3）
- [ ] 几何体厨房：地板 + 两面墙 + 冰箱 / 烤炉 / 组装台 / 出餐口四个工位（方块即可）（指南 §2 §7）
- [ ] 几何体角色：球 + 胶囊 + 方块，走路靠代码控制手脚旋转，零美术（指南 §7）
- [ ] 贴地椭圆 Blob Shadow（不是实时阴影）（指南 §6）

⚠ **指南 §0 的三条红线**（不加物理组件 · 不开实时阴影 · 不指定 skybox cubemap）
直接对应 A5 实测出来的包体数字，越过去 A5 那笔账就白算了。
...
- [x] 顾客头顶五档情绪图标，由耐心剩余比例映射（`GDD.md` §12.5）—— 2026-09-24，与耐心圈合一
- [ ] 人物模型的情绪动作（不耐烦 / 发怒）：Kenney 模型没有这类动画，**换模型时一起做**，现在先靠头顶表情
- [x] 顶部评价弹窗 + 点单台电脑查看评价（2026-09-24）。头像暂用色块 + 身份首字，换模型出肖像后替换
- [ ] 客流曲线按 M1 的难度表推进
...
- [x] 盘子有限 → 洗碗池「泡 → 按住刷 → 晾」（2026-09-24，`Station_Sink` 一个大池子）
- [x] 取熟肉时自动装盘（没盘取不了肉），组装台放面包、配料不占盘（2026-09-24）
- [x] 手上最多两样不同配料，肉饼单独拿；`sim.ts` 同步改，后期客流间隔 15.5→14s 抵回完成率、`IDEAL_SERVED` 重标（2026-09-24）
- [x] 汉堡按层叠成 3D（`BurgerStack.ts`，素材克隆场景里现成的食物模型；洋葱 / 酸黄瓜 / 培根暂用换色替身）（2026-09-24）
- [ ] 洗盘两种失误：提前松手 = 脏盘 / 叠 >3 个变慢、撞墙全摔（GDD §12.5，未做）
- [ ] **顾客目击系统**：事故与发泄被在场顾客看到 → 情绪降档 + 吐槽。洗盘失误、发泄都挂在它上面，先于二者做
- [ ] 薯条线：炸锅火候 + 出锅变凉（衰减数值交 `sim.ts` 扫）
- [ ] 双层汉堡（占两个烤位）
- [ ] 暴怒后果分级：投诉 / 退款 / 走人，另有砸坏长凳（座位减少，打烊后决定修不修）
- [ ] 发泄行为：情境长按 0.6s（垃圾桶 / 冰箱 / 怼暴怒顾客），4s 移速 +30%，被看见才有代价
- [ ] 卡片 schema 加 `lines.witness` / `lines.retort`，补写已有 25 张 + 21 张夹具
- [ ] 奇葩订单（半暗示，天数解锁）—— 机制在这里，量产台词归 M5
- [ ] ⏸ 饮料：点一下开始接、晚了溢出，排在本里程碑之后
- [ ] 烤炉过火 → 烤糊 → 起火 → 灭火器
- [ ] 催单、差评、卫生检查
- [x] 外卖（2026-09-24）：点单台电脑接单 / 拒单，接了同堂食看得见食材，前台东段 `Station_Delivery` 交货。
      定案：拒单或挂着不理 3 星、接了超时 1 星并作废、交错 1 星。**骑手小人还没做**（现在只有取餐口上的圈）
